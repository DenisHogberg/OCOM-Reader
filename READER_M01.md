...
**Note on numbering:** this "M01" is a new counter for the Reader↔Vector integration track
...
Make Reader fully compatible with `docs/contracts/vector-reader-contract.md` (published by
the Vector repository, Contract Version 1.0) without changing Vector's behavior in any way.

## What changed

Nothing in `~/Downloads/Vector` was touched — verified (`git status`/`git diff` against
Vector's repo show no modifications from this work). Everything below is new, additive
...
- **`pyproject.toml`** — added `pyyaml>=6.0` as a dependency. Reader had no YAML parsing
  ...
  existing codebase); Vector's objects are YAML-frontmatter + Markdown, so this is required
  to read them at all.
- **`src/ocom_reader/vector_integration/`** (new package, 4 files):
  - `models.py` — `VectorStatement` and `VectorMeeting`, Pydantic models covering exactly
    the fields `vector-reader-contract.md` governs. Both set `model_config =
  ...
  - `loader.py` — `parse_frontmatter()`, `load_statements()`, `load_meetings()`,
    `find_current_meetings()`. Read-only; tolerates non-Vector Markdown files
...
- **`src/ocom_reader/cli.py`** — added a new `vector` subcommand (`vector show <path>`,
  `vector search <root> --signal {metric,question,risk,task,decision}`), dispatched
  independently of Reader's existing `--repo`/workspace machinery (a Vector repository is a
  separate external input, not "the repository being read" in Pipeline A's sense — see
  `vector_integration/__init__.py`'s docstring for why this wasn't grafted onto the
  existing `Reader`/`RetrievalEngine` facade).
- **`tests/test_vector_integration.py`** (new, 14 tests).
...
- **Pipeline B** (`adapters/`+`normalizers/`+`core/object.py`) is object-shaped
  ...
  fit for a producer (Vector) that already ships a typed, versioned object model of its own.

Vector's Statement/Meeting objects already satisfy their own contract; Reader's job is to
consume that contract precisely, not re-normalize it through either existing pipeline's
different assumptions. A dedicated `vector_integration/` package models the contract
directly.

## Tests performed

**Isolated (synthetic fixtures, 10 tests)** — `tests/test_vector_integration.py`:
...
future field is ignored, not rejected; non-Vector Markdown is skipped, not fatal;
...
**Real data (4 tests, run directly against `~/Downloads/Vector`, not a copy)**:
- `load_meetings`/`load_statements` against Vector's entire `ai/staging/` tree — 10 real
...
## Confirmation against `vector-reader-contract.md`

| Contract requirement | Status |
|---|---|
| Read mandatory fields | ✅ `VectorStatement`/`VectorMeeting` model exactly the contract's mandatory set |
...
| Statement Kind + Detected Signals display | ✅ `ocom-reader vector show` |
| Per-signal search filter | ✅ `ocom-reader vector search --signal {metric,question,risk,task,decision}` |
| Verified against real M07 data | ✅ direct reads of `~/Downloads/Vector/ai/staging/`, not copied fixtures |

## Task 5 — explicit confirmation

No code anywhere in `vector_integration/` compares `detected_signals` against a fixed
...
- No rich Signal Search UI (M02) — `vector search` is a working, tested, minimal CLI
...
- No integration into Reader's existing `Reader`/`RetrievalEngine`/`AnswerComposer`
  facade (Pipeline A) — `vector` is a fully independent CLI subcommand today. Whether/how
