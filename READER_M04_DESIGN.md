...
starting M04. Grounded in the actual current state of `~/Downloads/Companion`, checked
...
the same way Companion's own `ai/staging/` review already works
(`docs/ai-collaboration.md`'s write-back governance). This is a review queue, not a
promotion pipeline — reflected directly in the CLI name (`companion review`, decided below)
...
>
> **Reader MAY** group, sort, and visualize contracted Companion data.
>
> **Reader MUST NOT** create Promotion Candidates, Promotion Scores, Promotion Labels,
> or any derived workflow state that is absent from the Companion Contract.

This is the concrete, checkable form of a principle this project has followed since
M01 without stating it this bluntly: Companion owns analysis; Reader owns navigation and
...
touches `companion_integration/` — checked against the design above, not just asserted:

1. **No new contract fields.** Reader consumes only what `companion-reader-contract.md`
...
2. **No write-back to Companion.** Read-only, no exceptions. **Checked**: Data Flow above
...
3. **No hidden heuristics.** If a grouping needs a guess Reader can't ground in an exact,
   ...
   the rejected alternative (reimplementing Companion's signal-combination table) was
...
4. **Testing discipline, in order**: unit tests (synthetic fixtures) → full regression
   suite → manual CLI smoke test against the real `~/Downloads/Companion` repository — the
...
5. **Document every contract discrepancy.** Any field Reader depends on that isn't in
   the formal contract must stay explicitly flagged (in `docs/companion-integration.md`
   and the milestone report) until Companion formally updates the contract — never let it
   quietly become "assumed stable." **Checked**: `meeting_date`'s beyond-contract status
   is restated in Compatibility and Companion Contract assumptions below, not just in
   M03's report.

## The finding that shapes everything below: Companion has no persisted Promotion Candidate data at all

Checked directly before writing anything else in this document:

```
$ find ~/Downloads/Companion -iname "*promot*"
./ai/pipelines/M05_PROMOTION_READINESS.md
$ ls ~/Downloads/Companion/schemas/     # no promotion/candidate schema
...
Companion's own `M05_PROMOTION_READINESS.md` (Companion-side numbering — a different M05 than
...
data source, and not as a reimplementation of Companion's own candidate-labeling logic (see
Design Principle above).

## Public API

Proposed additions to `companion_integration/`, all pure functions over already-loaded data
...
CLI: **`companion review <root>`** (decided #1 — not `companion promotion`, not
`companion candidates`; Reader doesn't promote or decide, it reviews). A later,
explicitly out-of-scope-for-M04 filter flag (`companion review --only task,risk`) was
...
No write path. No new object creation. No call into anything resembling Companion's
`ingest_transcript.py`/`signal_detection.py` logic — Reader does not re-derive or
second-guess `statement_kind`, it only displays the value Companion already computed and
persisted.
...
- **No new Companion field required** for the core classification. `statement_kind` and
  `detected_signals` are both already in `companion-reader-contract.md` v1.0.
- **`meeting_date` is reused, not newly added.** The Meeting-grouping/ordering decision
  (#4) depends on `CompanionMeeting.meeting_date` — the same field M03 already introduced
...
- **Old-style Statements** with no `statement_kind` at all (checked: none currently
  ...
  Statement since Companion's own M04, but the field is `Optional[str]` and nothing
...
| `statement_kind` holds a value outside the 6 known ones (future Companion enum extension) | Goes to its own ad-hoc bucket by that literal value, not an error, not silently dropped into `other` | mirrors the contract's "ignore unknown, don't reject" spirit already used for unknown Statement *fields*; here applied to an unexpected *value* of a known field |
| **`statements` passed in without first calling `filter_to_current_meetings`** | Same Meeting appears as several duplicate sections, once per Companion reindex run | this is not a new failure mode invented for M04 — it is the exact bug M03 found and fixed for Object Navigation; `render_promotion_review` inherits it if a caller skips the filter, which is why the filter is stated as REQUIRED in Public API, not optional |
...
## Companion Contract assumptions

- Relies on `statement_kind` being a single, closed-vocabulary value chosen by Companion's
...
- Relies on `detected_signals` **only** for display (showing which signals fired
  ...
  never for grouping or candidate-labeling logic. Companion's own
  `M05_PROMOTION_READINESS.md` signal-combination → label table is explicitly **not**
  reimplemented here — doing so would make Reader a second implementation of Companion's
...
- Does **not** assume Companion will ever publish a real Promotion Candidate schema. If it
...
and new Reader versions can run side by side against the same Companion data with no
...
- An unexpected `statement_kind` value (simulating a future Companion enum extension) gets
...
**Real data**, against `~/Downloads/Companion` directly (not copied):
- Cross-check `task_signal` count for `MTG-00000000-DEMO` against the number already
  ...
  Companion's own `M06_RESULTS.md` reporting `task_signal` count **7** for this exact
...
- `companion review ~/Downloads/Companion` (whole repo root) — confirm group counts sum
...
- `companion review ~/Downloads/Companion/ai/staging/MTG-00000000-DEMO` (single meeting root)
...
1. **Naming.** Decided: `companion review`, not `companion promotion` — Reader reviews, it
...
2. **Scope boundary on "candidate" richness.** Decided: `statement_kind` +
   `detected_signals` only, display-only. Companion's `M05_PROMOTION_READINESS.md`
   signal-combination → label table is explicitly rejected as a Reader-side
   reimplementation — it would make Reader a second implementation of Companion's business
