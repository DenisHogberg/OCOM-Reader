...
Read top to bottom as someone who has never seen this repository, OCOM, or Companion:
...
- **Is it clear how Reader differs from Companion?** No — not partially, **totally**.
  Checked directly: `grep -i companion README.md` returns exactly one hit, and it's the
  phrase "no companion store" — describing the *absence* of companion-search technology in
  the original MVP pipeline. The word never once refers to the actual Companion
  repository. Someone who has heard of Companion and lands here has no way to learn from
  this document that Reader reads Companion's data, is a separate codebase, and doesn't
  modify anything Companion produces. This is the single largest gap found in this audit.
- **Is it clear what problem Reader solves?** Partially, but for an outdated scope.
  ...
  about everything built since: reading and reviewing a *separate* Companion repository's
  operational data (10 `companion` subcommands), working across multiple repositories by
...
| Examples | Minimal, narrow | Four one-line `ask`/`search`/`related`/`explain` examples exist. Nothing demonstrates any of the other 20 real CLI subcommands (`repo`, `plugin`, `web`, `companion` ×10). |
| CLI reference | **Missing** | No comprehensive list of Reader's actual command surface exists anywhere in the README. |
| Architecture | Present, disproportionately so | ~180 of 258 lines (roughly 70% of the document) are the historical Phase 1-5 / `core`/`interfaces` architecture narrative — thorough, but so large it pushes everything else (installation, CLI, Companion integration) below the fold. |
| Documentation (pointer to `docs/`) | **Missing** | No summary or index of `docs/architecture/` (46 files), `docs/companion-integration.md`, or any of the seven root-level `READER_*.md` reports — including `READER_STATUS.md`, which was written specifically to serve as an entry point, but is never linked from the actual entry point. |
...
- **Outdated or incomplete commands**: not literally wrong (`ask`/`search`/`related`/
  ...
  `completion`, and `companion` [10 subcommands]).
...
- **Overall inconsistency**: the document's own self-description — `pyproject.toml`'s
  ...
  commits, four Companion-integration milestones, three Product Readiness items later).
...
| Integration with Companion | **Missing entirely** — the largest and most consequential gap in this audit |
...
not a separate section: "Reader reads Companion's data; it is a separate
...
dedicated Companion section someone may never scroll to.
...
## CLI
   ...
   four groups — ask/search/explain/related, repo, companion, web — each
   with one real example, plus a single explicit pointer: "run
   `ocom-reader --help` or see docs/companion-integration.md for the
...
## Working with Companion
   (what Companion is, in one sentence; what Reader can and cannot do with
   it — read-only, no promotion, no write-back; a link to
   docs/companion-integration.md for the full contract.)
## Architecture
...
## Documentation
   (an index: docs/companion-integration.md, docs/architecture/, and the
...
- It matches the structure already proposed for Stage 2, with three concrete
  ...
  2. **The Companion distinction folded into the opening paragraph itself**, not
     deferred to the "Working with Companion" section — the First Impression findings
...
- **CLI section is curated, not exhaustive**, by design: a full flag-by-flag
  ...
  benefit over pointing to `--help` and `docs/companion-integration.md` directly.
...
- **Every "missing" item from Section 4 above has an explicit home** in this
  ...
  `READER_ROADMAP_REVIEW.md`, `docs/companion-integration.md`, `LICENSE`, and
...
4. `ocom-reader companion stats path/to/companion-repo` (run against the real
   `~/Downloads/Companion` repository, substituted for the placeholder) — **worked**,
   clean, short, immediately legible output (Meetings/Statements/Tasks/Metrics/Risks/
   Decisions/Questions counts).
5. `ocom-reader companion review path/to/companion-repo` — **worked**, produced exactly what
...
`README.md` itself.** The one placeholder (`path/to/companion-repo`) is self-explanatory
in context and was resolved correctly on the first try using a real Companion repository.
...
Companion repository → get a genuinely useful first result, entirely from `README.md`
alone.
