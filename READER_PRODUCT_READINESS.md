...
- **Milestones implemented**: the Adapter/Normalizer core (Phases 1-5), the Reader MVP
  ...
  Layer (M021 Phase 1), and the Vector integration (Reader M01-M04: Contract Compliance,
  Signal Explorer, Object Navigation, Promotion Review).
- **CLI commands**: `ask`, `search`, `related`, `explain`, `repo` (4 sub-commands),
  `plugin` (5 sub-commands), `web`, `completion`, and `vector` (10 sub-commands) — 24
...
| **Versioning** | **Needs Improvement** | `pyproject.toml` uses semver-shaped syntax correctly, but the value has been static since the file's first commit despite four completed, tested Vector-integration milestones alone (plus everything else since `v1.0.0`). Present as a mechanism, not maintained as a signal. |
...
against live Vector data where applicable); `pyproject.toml`'s basic mechanics
...
- **Is it clear how Reader differs from Vector?** No — the distinction doesn't exist in
  this document at all. `README.md` contains zero occurrences of "Vector." Someone who
  has heard of Vector (a separate, real, sibling project) and lands on this repository
  has no way to learn from the README that Reader *reads* Vector's data, doesn't
...
- **Is it clear how to get started?** Mostly, once you reach it — `pip install -e .`
  ...
  only — there is no equivalent quick-start for the Vector integration's 10 commands
  anywhere in the README.
- **Is it clear what capabilities already exist?** No, for the Vector integration
...
"Reader vs. Vector" callout early, linking to `docs/vector-integration.md` and
...
- README redesign per Section 3's findings: lead with plain-language "what/why,"
  explicit Reader-vs-Vector distinction, a real `## Quick Start`, fixed/removed
...
| README redesign (Quick Start, Reader-vs-Vector, fix broken links) | High — this is the actual first thing every future user or contributor sees | Medium — a rewrite, not a patch, to do properly | Low — content-only, no functional risk, but easy to under-scope if rushed | **5** |
