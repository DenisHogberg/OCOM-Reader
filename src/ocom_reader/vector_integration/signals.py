...
Per docs/contracts/vector-reader-contract.md: `detected_signals` is a set of
...
growing as Vector ingests more real transcripts (see the contract's own
evidence: 19 distinct combinations across just 5 transcripts in Vector's
M07 validation).
...
Contract change — vector-reader-contract.md governs Statement's *data*
fields, not how Reader chooses to print them. M01's own tests for the old
format were updated alongside this change (see tests/test_vector_integration.py).
"""
...
def filter_by_signal(statements: list[VectorStatement], signal: str) -> list[VectorStatement]:
    ...
    if signal not in KNOWN_SIGNALS:
        raise ValueError(
            ...
            f"per vector-reader-contract.md's closed signal vocabulary."
        )
    return [s for s in statements if s.has_signal(signal)]


def signal_counts(statements: list[VectorStatement]) -> dict[str, int]:
...
def multi_signal_count(statements: list[VectorStatement]) -> int:
    return sum(1 for s in statements if len(s.detected_signals) >= 2)


def zero_signal_count(statements: list[VectorStatement]) -> int:
    return sum(1 for s in statements if not s.detected_signals)


def render_meeting_summary(statements: list[VectorStatement]) -> str:
...
def render_signal_browser(statements: list[VectorStatement]) -> str:
...
def render_statement(stmt: VectorStatement) -> str:
