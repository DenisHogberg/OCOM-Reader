# Reader Status — Vector Integration
...
- ✓ **M01 — Contract Compliance** (`READER_M01.md`) — reads Vector's Statement/Meeting
  objects per `docs/contracts/vector-reader-contract.md` v1.0; forward/backward
...
## Current capabilities (CLI: `ocom-reader vector <command>`)
...
Implementation: `src/ocom_reader/vector_integration/` (`models.py`, `loader.py`,
`signals.py`, `query.py`, `stats.py`, `navigation.py`, `promotion.py`). Tests:
`tests/test_vector_integration.py` — **512 passing** as of M04 (all against both
synthetic fixtures and Vector's real, live repository at `~/Downloads/Vector`, not
copies).

## Contract dependencies

- **Contract v1.0** (`docs/contracts/vector-reader-contract.md`) — the formal,
...
- **`Meeting.meeting_date`** *(temporary — beyond Contract v1.0)* — introduced in M03
  ...
  contracted field substitutes for it. Needs either a formal v1.1 addendum or Vector
  telling Reader to source the date another way.
- **`VectorObject`** *(temporary — beyond Contract v1.0)* — Vector's common
  ...
  shape exists yet, only Vector's own internal `docs/object-model.md`.

Both exceptions are read defensively (optional, tolerant of absence) and are restated
in `docs/vector-integration.md` on every milestone that depends on them — not something
that quietly became "assumed stable."

## Known limitations

- **`relationships` not populated.** All 6 real Vector objects (`PTN-*`, `EMP-*`) have
...
- **Promotion Candidate data is absent in Vector entirely.** No schema, no persisted
  object, no field — checked directly (`find`/`grep` across Vector's repo) before M04's
  design. Vector's own `M05_PROMOTION_READINESS.md` is analysis-only ("no objects
...
- **Speaker identity not resolved.** Every Statement's `speaker` field is Vector's raw
  diarization label (`"Speaker 1"`, `"Speaker 2"`) in all real data observed — searching
  `speaker:Denis` finds nothing until Vector resolves speaker identity on its side. A
  Vector-side gap Reader inherits, not something Reader can work around.
...
2. **Reader is read-only.** No write-back into Vector, no exceptions.
...
4. **Testing discipline, in order:** unit tests → full regression suite → manual CLI
   smoke test against the real `~/Downloads/Vector` repository. Not optional — this
   sequence is what caught M03's real reindex-duplication bug, which no synthetic
   fixture surfaced.
5. **Document every contract discrepancy** until Vector formally closes it.
...
future milestone touching `vector_integration/` should get a `READER_M0X_DESIGN.md`
first (Goal / Public API / Data Flow / Complexity / Compatibility / Failure Modes /
Vector Contract assumptions / Migration / Test plan / Open Questions), the same way M04
