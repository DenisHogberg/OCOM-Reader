...
Legacy Development (M006–M021)
   ...
   product — see "Note on early tags" below.
        │
        ▼
Reader Product — Vector Integration (M01–M04)
   A separate, restarted milestone count for Reader's Vector-integration
...
- Vector integration (Reader M01-M04): read-only consumption of a Vector repository's
  Statement/Meeting/object data per `vector-reader-contract.md` v1.0, forward/backward
  compatible by construction.
- 10 new `ocom-reader vector` subcommands: `show`, `search`, `signals`, `summary`,
...
- `src/ocom_reader/vector_integration/` package: `models.py`, `loader.py`, `signals.py`,
  `query.py`, `stats.py`, `navigation.py`, `promotion.py`.
- A binding Design Principle for all future Vector-integration work (established in
  M04): Reader must not infer new semantic objects, create Promotion Candidates/
  Scores/Labels, or otherwise duplicate Vector's own business logic — it may only
  group, sort, and visualize data Vector has already contracted.

### Changed

- `vector_integration.navigation.linked_meeting_ids()`'s signature changed from
  ...
  `vector` CLI command's behavior or output changed.
...
- A real reindex-duplication bug in Object Navigation (M03): a single real Meeting
  reprocessed multiple times by Vector's own idempotent-import pipeline (same
  ...
  `filter_to_current_meetings()`, reusing Vector's own `supersedes`-chain resolution.
...
- `docs/vector-integration.md` — supported contract version, compatibility
