...
state (both `~/Downloads/OCOM-Reader` and `~/Downloads/Vector`) while writing this, not
...
**What Reader can do after M04** — 10 `vector` subcommands (`show`, `search`, `signals`,
`summary`, `stats`, `object`, `mentioned-in`, `relationships`, `timeline`, `review`),
512 passing tests, all exercised against Vector's real repository in addition to
synthetic fixtures:
...
**User tasks already covered**: "what was said about X, and how confident is Vector in
it," "show me everything Vector's pipeline flagged as a possible task/decision/risk,
...
- **No CI, no LICENSE, no CHANGELOG.** `find .github -type f` → doesn't exist.
  ...
  artifacts.
- **The Vector integration is invisible from every other Reader surface.**
  `grep -n vector src/ocom_reader/web/api.py` → no matches. Same for
  `interactive.py` (the REPL). `README.md` → zero mentions of Vector, Statement,
  or `READER_STATUS.md`. Someone opening this repository fresh via its own front
  door would not discover that 10 commands and 512 tests of Vector integration exist.
- **Every `vector` command takes a raw filesystem path, every time.** Reader already
  has a named, persistent multi-repository mechanism (`ocom-reader repo add/use`,
  M016) — `vector_integration` was never wired to it. `vector review ~/Downloads/Vector`
  works; there is no `vector review` that resolves an active repo by name the way
  `ask`/`search`/`explain` already can.
- **Real Vector production data is narrow.** `objects/` has real files in only 2 of 12
  ...
  all empty). Every real Statement/Meeting seen so far carries `tenant: vector-primary`
  — multi-tenancy is a schema field, never yet exercised with a second real value.
  Speaker identity is unresolved on Vector's side (`speaker_resolved: false`
  everywhere), so `speaker:` search has never matched a real name.
- **Promotion Candidate data structurally doesn't exist in Vector** (confirmed during
...
|---|---|---|
| "Browse my Vector repo without retyping the path every time" | No | **Real, small.** The fix (wire into `repo`/`WorkspaceManager`) is mostly plumbing — the mechanism already exists and is proven in production for `ask`/`search`/`explain`. |
| "Search Statement *text*, not just signal/speaker/meeting metadata" | No | **Real, contained.** `query.py` filters by structured fields only; nothing greps `Statement.text`. Needs no new Vector field, no new contract dependency — the same "presentation over already-contracted data" discipline M04 followed. |
| "See this in the Web UI, not just the CLI" | No | **Real, but larger.** `web/api.py` has zero Vector wiring. Meaningful effort, and its value is capped by how few people are actually using this against real data yet (one tenant, one real company). |
...
| "See a richer 'this looks like a Task with a KPI' label, not just `statement_kind`" | No | **Not a Reader gap.** Blocked entirely on Vector publishing real Promotion Candidate data — M04's design review already established Reader must not invent this itself. No Reader milestone closes this. |
| "Navigate real relationships/aliases between objects" | Only against synthetic fixtures | **Not a Reader gap.** Blocked on Vector's real objects actually having `relationships`/`alias:` tags populated — 0 of 6 real objects do. More Reader-side relationship-browsing code wouldn't produce anything new to look at. |
| "Use this across two different companies' Vector repos" | Technically (pass a different path) but never actually exercised | **Validation gap, not a build gap.** The multi-tenant field exists; nothing has ever tested it with a second real tenant. |
...
require Vector-side changes Reader can't make happen (relationship data, speaker
...
1. **The stuff already built isn't finished being integrated.** No CI, no license, no
   changelog, and the entire Vector integration is undiscoverable from Reader's own
...
2. **Real Vector data hasn't grown enough to justify deeper analytical features.** M03's
...
3. **The two gaps that ARE real and buildable today (text search, workspace wiring) are
   small, not milestone-scale.** Neither needs a new Vector contract field, new
...
- **New UI (Web/TUI).** Real gap, but premature — Reader's existing Web UI (M018)
  already exists and has zero Vector wiring; extending it is meaningful work whose
  payoff is capped by how little real multi-user usage exists yet. Defer.
- **Full-text search over Statement.text.** Real, contained gap — no Vector-side
...
- **Multi-repository support.** The infrastructure (`WorkspaceManager`, `repo`
  ...
  to `vector_integration`. This is the highest-leverage small change available — it
  reuses proven infrastructure, fixes a real daily friction point (always retyping a
  path), and requires zero new Vector data or contract changes. Not really "M05"-sized;
  more an integration-debt fix.
- **Plugin system / API.** Both exist (M017, M018) but are unrelated to Vector
...
- **What actually follows from the current state, not from this list**: fixing the
  README/CI/LICENSE/CHANGELOG gap, and wiring `vector` into the existing workspace
...
2. **Discoverability**: a short Vector-integration section in `README.md` linking to
   `READER_STATUS.md` — currently a zero-cost fix for a total gap.
3. **Small integration-debt fix**: wire `vector` subcommand's `root` argument to
...
Only after that — and only if real usage or real Vector data growth actually surfaces
a need — would a genuine M05 (most likely full-text search, since it's the one
identified gap that doesn't depend on Vector producing more data) be justified. Until
...
- **Path B (New Features, M05/M06/...)** is the path the evidence above argues against
  ...
  Vector data) haven't happened yet. Sequencing B before A risks repeating the exact
...
- **Path C (Productization — PyPI, TUI/Web UI, API, Plugins)** already has real
  ...
  future path so much as "finish wiring Vector into infrastructure that already exists"
...
**A now, then B or C once A's prerequisites are met and either real Vector data grows
