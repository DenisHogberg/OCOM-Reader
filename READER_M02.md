...
**Builds on:** `READER_M01.md` (Contract Compliance) — same `companion_integration/` package,
...
- **`companion_integration/models.py`** — added `SIGNAL_DISPLAY_ORDER` (task, metric, risk,
...
- **`companion_integration/signals.py`** — added `signal_counts()`, `multi_signal_count()`,
  ...
  change, scoped" below.
- **`companion_integration/query.py`** (new) — `parse_query()`/`apply_filters()`/`search()`:
...
- **`companion_integration/stats.py`** (new) — `compute_stats()`/`render_stats()` (Task 5 in
  the milestone's own numbering — "Statistics").
- **`cli.py`** — `companion search` extended with an optional `query` positional (M01's
  `--signal` flag kept working, mutually exclusive with `query`); three new subcommands:
  `companion signals <root>`, `companion summary <root>`, `companion stats <root>`.
- **`docs/companion-integration.md`** (new) — Task 6: supported contract version,
  compatibility, explicit limitations (including the honest one: `speaker:` search matches
  nothing useful yet, because Companion's real data never has a resolved speaker name — see
  below), CLI examples for every command.
- **`tests/test_companion_integration.py`** — 12 new tests (26 total in the file), covering
  every M02 function both in isolation and against Companion's real M07 data.
...
change**, not a Contract change: `companion-reader-contract.md` governs Statement's *data*
...
`test_render_statement_with_no_signals`) — nothing about the underlying `CompanionStatement`
model or the contract's guarantees changed.

## An honest limitation, surfaced by actually running Task 4 against real data

`speaker:Denis` (a name) matches **zero** Statements in Companion's real corpus — verified
directly, not assumed. Every real Statement's `speaker` field is still a raw diarization
label (`"Speaker 1"`, `"Speaker 2"`) because Companion's own `speaker_resolved` is `False`
throughout its current data (documented in Companion's own `docs/ai-collaboration.md` roadmap
as not yet built). The filter itself works correctly — it would match `speaker:"Speaker 1"`
today — but the milestone's own example (`speaker:Denis`) can't succeed until Companion
resolves speaker identity on its side. Documented plainly in `docs/companion-integration.md`
...
**Real data (4 new, against `~/Downloads/Companion` directly)**:
...
including: `companion summary` on a real Meeting; `companion stats` on the full staging tree
...
| Criterion | Status |
|---|---|
| Reader shows signals | ✅ `companion show` (Signal View), `companion summary` (Meeting Summary), `companion signals` (Browser) |
| Reader can search by them | ✅ `companion search` — single-signal (M01-compatible) and combinable `signal:`/`speaker:`/`meeting:` queries |
| Reader can build summary statistics | ✅ `companion stats` |
| Contract v1.0 compatibility preserved | ✅ no field added/removed on `CompanionStatement`/`CompanionMeeting`; `extra="ignore"` untouched; full M01 test suite (backward/forward-compat, real mixed old/new data) still passes unmodified in behavior, only two display-format assertions updated to match the intentional Task 3 change |
...
- No resolution of the `speaker:` search limitation above — that requires a change on
  Companion's side (speaker identity resolution), not Reader's.
